...
class AuthManager {
    ...
    async getUsers(page = 1, limit = 10) {
        ...
        const safeLimit = Math.min(100, Math.max(1, parseInt(limit) || 10)); // Max 100 per page
        const offset = (safePage - 1) * safeLimit;

        // Use template literal for LIMIT/OFFSET since MySQL2 doesn't support parameterized LIMIT
        const users = await database.query(
            `SELECT id, username, email, full_name, role, is_active, max_messages_per_day, max_contacts_per_session, created_at FROM users ORDER BY id LIMIT ${safeLimit} OFFSET ${offset}`
        );
