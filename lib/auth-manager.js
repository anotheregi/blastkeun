...
class AuthManager {
    ...
    async getUsers(page = 1, limit = 10) {
        ...
        const users = await database.query(
            'SELECT id, username, email, full_name, role, is_active, max_messages_per_day, max_contacts_per_session, created_at FROM users ORDER BY id LIMIT ? OFFSET ?',
            [limit, offset]
        );
        
        const total = await database.query('SELECT COUNT(*) as count FROM users');
