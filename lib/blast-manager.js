...
class BlastManager {
    ...
    async setupClient() {
        try {
            ...
            // Try to use chrome-aws-lambda for Railway
            try {
                const chrome = require('chrome-aws-lambda');
                puppeteerConfig.executablePath = await chrome.executablePath;
                puppeteerConfig.args = chrome.args.concat(puppeteerConfig.args);
                console.log('✅ Using chrome-aws-lambda for Railway');
            } catch (chromeError) {
                console.log('⚠️  chrome-aws-lambda not available, using system Chrome');
    ...
    getConnectionStatus() {
        return {
            ...
            mode: this.isAuthenticated ? 'AUTHENTICATED' : 'MOCK_MODE',
