...
// Import puppeteer-core for Railway compatibility
let puppeteer;
try {
    puppeteer = require('puppeteer-core');
} catch (error) {
    console.log('puppeteer-core not found, using default puppeteer');
...
class BlastManager {
    constructor() {
        ...
        // Check if running on Railway (serverless environment)
        const isRailway = process.env.RAILWAY_PROJECT_ID || process.env.RAILWAY_ENVIRONMENT;

        if (isRailway) {
            console.log('🔧 BlastManager initialized (Railway Mock Mode)');
            this.isAuthenticated = false; // Force mock mode for Railway
        } else {
            console.log('🔧 BlastManager initialized (Production Mode)');
            // Initialize client asynchronously only if not Railway
    ...
    async setupClient() {
        try {
            ...
            // Configure Puppeteer for Railway
            ...
            try {
                ...
                console.log('✅ Using @sparticuz/chromium for Railway');
            } catch (chromiumError) {
                console.log('⚠️  @sparticuz/chromium not available, using system Chrome');
                // Fallback to system Chrome or puppeteer default
            ...
            console.log('🔄 Falling back to mock mode for Railway compatibility');
            this.isAuthenticated = false; // Fallback to mock mode
        }
    }
    ...
    async sendMessage(phone, message, contact, mode) {
        try {
            ...
            // Wait for client initialization if not ready
            if (this.initPromise) {
                await this.initPromise;
            }

            if (!this.client || !this.isAuthenticated) {
                console.log('🔄 WhatsApp not authenticated, using mock mode');
                // Fallback to mock mode for Railway
    ...
    getConnectionStatus() {
        ...
        return {
            ...
            mode: this.isAuthenticated ? 'AUTHENTICATED' : (isRailway ? 'RAILWAY_MOCK' : 'MOCK_MODE'),
            qrAvailable: !!this.qrCode,
            info: this.client?.info || null,
            fallbackMode: !this.isAuthenticated,
            environment: isRailway ? 'railway' : 'other'
        };
    }
